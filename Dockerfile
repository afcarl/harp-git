...
ENV DOCKER_RUN "echo $(docker -rm -f BACKEND_BLOGDSPEED) $(docker run -d --restart=always --name=BACKEND_BLOGDSPEED --env HAPROXY_DOMAIN=blog.dspeed.eu --env REPO=https://github.com/DirektSPEED/blog_dspeed  dockerimages/harp-git)"
